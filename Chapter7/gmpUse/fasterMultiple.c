...
int main(void){
	...
	#ifdef MUTIPLEPROCESS
	{
		#define N 4
		int limit[] = {0,30,34,37,40};
		pid_t pid[N];
		for(int i = 0;i < N;i++){
			pid[i] = fork();
			if(pid[i] < 0){
				...
				clock_t inner_start_t,inner_end_t;
				inner_start_t = clock();
				...
				inner_end_t = clock();/* ending encounter */
				int tempTime = (float)(inner_end_t-inner_start_t) / CLOCKS_PER_SEC;
				// printf("i:%d child_pid:%d running:%d\n", i,getpid(),tempTime);
				exit(tempTime);
			}
		...
		while((retpid = waitpid(pid[i++],&status,0)) > 0){
			if(WIFEXITED(status)){
				int gap = WEXITSTATUS(status);
				if(total_t <= gap){
					total_t = gap;
				}
				// printf("child terminated normally, pid:%d exit status:%lf__%d\n", retpid, total_t,gap);
			}else{
