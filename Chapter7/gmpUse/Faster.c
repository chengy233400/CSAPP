...
/* to store the selected number, and the length is constant decimal + 1 */
...
void init(const int w){
	selected = malloc(sizeof(int)*(decimal+1));
...
void generator_noniteration(const int w){
	...
	do{
		...
		if(mpz_cmp(sum_a,k_powerOf_10[w]) >= 0){
			/* 上枝剪枝算法 */
			mpz_add(sum_a,sum_a,numOf_powerRoots[1][selected[0]]);
			selected[1] += selected[0];/* 兼容进位算法 */
			selected[0] = 0;/* 调用进位算法 */
			goto iter;
		}
		...
		mpz_t temp_a;
		mpz_init_set(temp_a,numOf_powerRoots[cIndex][selected[0]]);
		mpz_add(temp_a,temp_a,sum_a);
		if(mpz_cmp(temp_a,k_powerOf_10[w-1]) < 0){
			/* bug: 无法判断是否选了0，故如果rest==0，则会去掉所有含零的排列 */
			/* 下枝剪枝算法 */
			mpz_add(sum_a,sum_a,numOf_powerRoots[1][selected[0]]);
			selected[1] += selected[0];/* 兼容进位算法 */
			selected[0] = 0;/* 调用进位算法 */
		...
		// 	/* cut precheck */
		// 	/* temp_a is max */
		// 	if(!preCheck(sum_a,temp_a,cIndex,rest,w)){
	...
	iter:
		if(selected[0] == 0){
			if(selected[cIndex] >= w){
				cIndex++;
			}

			int pIndex = 0;
			for(;pIndex < decimal;pIndex++){
				if(selected[pIndex] != 0) break;
			}
			
			rest = selected[pIndex];

			mpz_sub(sum_a,sum_a,numOf_powerRoots[pIndex][selected[pIndex]]);
			// gmp_printf("sub:%Zd\n",sum_a);
			selected[pIndex] = 0;
			selected[++pIndex]++;

			if(pIndex < decimal) mpz_add(sum_a,sum_a,powerRoots[pIndex][w]);
			// gmp_printf("add:%Zd\n",sum_a);
			int sumLeft = 0;
			for(int i = pIndex;i < decimal;i++){
				sumLeft += selected[i];
			}
			selected[0] = w-sumLeft+1;
			selected[1] = -1;
		}
		...
		selected[1]++;
		rest--;
		if(selected[1] > 0) mpz_add(sum_a,sum_a,powerRoots[1][w]);

	}while(selected[decimal] == 0);
	mpz_clear(sum_a);
}
#endif
#ifdef MODE_TWO
